...
export default function MissionOverview() {
  ...
  const activeMissions =
    missionsData?.filter((m) => m.status === "in-progress") || [];
  const scheduledMissions =
    missionsData?.filter((m) => m.status === "scheduled") || [];
  const completedMissions =
    missionsData?.filter((m) => m.status === "completed") || [];
  ...
  const formatMissionTime = (mission: Mission) => {
    if (mission.status === "in-progress" && mission.startTime) {
      ...
      const isToday =
        new Date().toDateString() === scheduledTime.toDateString();
  ...
  const calculateRemainingTime = (mission: Mission) => {
    ...
    const completedMins =
      ((mission.completionPercentage || 0) / 100) * totalEstimatedMins;
              ...
              <div className="absolute bottom-4 left-4 bg-white bg-opacity-90 shadow-md rounded-md p-4 max-w-md">
                ...
                <div className="flex items-center text-sm mb-2">
                  ...
                  {selectedMission.completionPercentage ? (
                    <>
                      ...
                      <span>
                        Est. completion: {format(new Date(), "h:mm a")}
                      </span>
                    </>
                  ) : null}
                </div>
                ...
                <div className="flex mt-3">
                  <Button
                    ...
                    onClick={() =>
                      handleMissionAction(selectedMission.id, "pause")
                    }
                  >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleMissionAction(selectedMission.id, "abort")
                    }
                  >
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="p-4">
          <Tabs defaultValue="active" onValueChange={setActiveTab}>
            ...
            <TabsContent
              value="active"
              className="space-y-3 overflow-auto max-h-[24rem]"
            >
              {missionsLoading ? (
                ...
                activeMissions.map((mission) => (
                  ...
                  >
                    ...
                    <Progress
                      value={mission.completionPercentage || 0}
                      className="h-1.5"
                    />
                  </div>
                ))
              ...
              {missionsLoading ? (
                ...
                scheduledMissions.map((mission) => {
                  const isStartingSoon =
                    ...
                    new Date(mission.scheduledTime).getTime() -
                      new Date().getTime() <
                      30 * 60 * 1000;
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-gray-500 text-sm mb-1">
                        ...
                        {mission.droneId
                          ? `Assigned: Drone #${mission.droneId}`
                          : "Pending Assignment"}
                        <span className="mx-1">•</span>
                        <span className="material-icons text-xs align-text-bottom mr-1">
                          schedule
                        </span>
                        Est. {mission.missionType === "perimeter" ? "30" : "45"}{" "}
                        min
                      </p>
                      <div className="flex mt-1">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-gray-500 hover:text-blue-600 h-6 px-1"
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-gray-500 hover:text-red-600 h-6 px-1"
                        >
                          <span className="material-icons text-xs mr-1">
                            close
                          </span>
                ...
                completedMissions.map((mission) => (
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm">
                      ...
                      {mission.endTime
                        ? formatDistanceToNow(new Date(mission.endTime), {
                            addSuffix: true,
                          })
                        : "Recently"}
                      <span className="mx-1">•</span>
