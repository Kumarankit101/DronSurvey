...
export default function SurveyReport() {
  const [selectedReport, setSelectedReport] = useState<SurveyReportType | null>(
    null
  );
  ...
  const prepareSummaryData = () => {
    ...
    const areaCoveredData = Object.keys(areaCoveredByMonth)
      .map((month) => {
                    ...
                    <TableBody>
                      {reports?.length === 0 ? (
                        ...
                        reports?.map((report) => (
                          ...
                          >
                            <TableCell>
                              {formatReportDate(new Date(report.date))}
                            </TableCell>
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                    ...
                    <div className="text-3xl font-medium">
                      {reports
                        ?.reduce(
                          (sum, report) =>
                            sum + parseFloat(report.areaCovered || "0"),
                          0
                        )
                        .toFixed(1)}{" "}
                      km²
            ...
            <div className="mt-4 space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-medium">
                    {getMissionName(selectedReport.missionId)}
                  </h2>
                  <p className="text-gray-500">
                    {formatReportDate(new Date(selectedReport.date))}
                  </p>
                </div>
                <Badge
                  variant="outline"
              ...
              {selectedReport.findings && (
                <div className="pt-2">
                  ...
                  {selectedReport.findings.issues &&
                  selectedReport.findings.issues.length > 0 ? (
                    <div className="space-y-2">
                      <h4 className="text-sm font-medium text-gray-700">
                        Issues Identified:
                      </h4>
                      <ul className="list-disc pl-5 text-gray-700">
                        {(selectedReport.findings.issues as string[]).map(
                          (issue, index) => (
