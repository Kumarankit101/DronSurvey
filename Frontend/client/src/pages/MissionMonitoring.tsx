...
export default function MissionMonitoring() {
  ...
  const handleMissionAction = async (action: string) => {
    ...
    try {
      await apiRequest({
        method: "PATCH",
        url: `/api/missions/${selectedMission.id}`,
        data: { action },
      });
  ...
  const executeConfirmedAction = async () => {
    ...
    try {
      await apiRequest({
        method: "PATCH",
        url: `/api/missions/${confirmAction.missionId}`,
        data: {
  ...
  const calculateTimeRemaining = (mission: Mission) => {
    if (!mission.startTime || mission.completionPercentage === undefined)
      return "Unknown";
                ...
                <div className="h-[500px] flex items-center justify-center bg-gray-50">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 max-w-md">
                      There are currently no active drone missions to monitor.
                      Start a mission from the Mission Planning page.
                    </p>
                    <Button
                      className="mt-4"
                ...
                <div className="relative h-[500px]">
                  ...
                  <div className="absolute bottom-4 left-4 bg-white bg-opacity-95 shadow-md rounded-md p-4 max-w-md">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                      ...
                      <div className="flex items-center text-gray-600">
                        ...
                        {missionLocation?.name ||
                          `Location ${selectedMission.locationId}`}
                      </div>
                      <div className="flex items-center text-gray-600">
                        <span className="material-icons text-sm mr-1">
                          category
                    ...
                    <div className="flex justify-between text-xs text-gray-500 mb-3">
                      <span>
                        {selectedMission.completionPercentage || 0}% Completed
                      </span>
                      <span>
                        {calculateTimeRemaining(selectedMission)} remaining
                      </span>
                    </div>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will immediately stop the drone mission and
                              return the drone to its home position. This action
                              cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Abort Mission?</AlertDialogTitle>
            <AlertDialogDescription>
              This will immediately stop the drone mission and return the drone
              to its home position. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
