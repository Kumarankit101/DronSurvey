...
export default function DroneList() {
  ...
  const handleDroneAction = async (droneId: number, newStatus: string) => {
    try {
      await apiRequest('PATCH', `/api/drones/${droneId}`, { status: newStatus });
      queryClient.invalidateQueries({ queryKey: ['/api/drones'] });
      
      toast({
        title: "Drone Status Updated",
        description: `Drone has been marked as ${newStatus}`,
      });
    } catch (error) {
      console.error('Failed to update drone status:', error);
      toast({
        title: "Update Failed",
        description: "Could not update drone status. Please try again.",
        variant: "destructive",
      });
    }
  };
                ...
                <TableBody>
                  {filteredDrones?.length === 0 ? (
                    ...
                    filteredDrones?.map((drone) => (
                      <TableRow key={drone.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => openDroneDetails(drone)}>
                        <TableCell className="font-medium">{drone.name}</TableCell>
                        <TableCell>{drone.model}</TableCell>
                        <TableCell>
                          <Badge variant="outline" className={`bg-${getStatusColor(drone.status)}-100 text-${getStatusColor(drone.status)}-800 border-${getStatusColor(drone.status)}-300`}>
                        ...
                        <TableCell>
                          <div className="flex items-center">
                            ...
                            />
                            <span className="text-xs font-mono">{drone.batteryLevel}%</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          {drone.lastMission ? (
                        ...
                        <TableCell>
                          {drone.locationLat && drone.locationLng ? (
                            <span className="text-xs font-mono">
                              {parseFloat(drone.locationLat).toFixed(4)}, {parseFloat(drone.locationLng).toFixed(4)}
                            </span>
                          ) : (
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {drone.status !== "available" && (
                                <DropdownMenuItem onClick={() => handleDroneAction(drone.id, "available")}>
                                  <span className="material-icons text-sm mr-2">check_circle</span>
                                  Mark as Available
                                </DropdownMenuItem>
                              )}
                              {drone.status !== "charging" && (
                                <DropdownMenuItem onClick={() => handleDroneAction(drone.id, "charging")}>
                                  <span className="material-icons text-sm mr-2">battery_charging_full</span>
                                  Send to Charging
                                </DropdownMenuItem>
                              )}
                              {drone.status !== "maintenance" && (
                                <DropdownMenuItem onClick={() => handleDroneAction(drone.id, "maintenance")}>
            ...
            <div className="mt-4 space-y-4">
              <div className="flex items-start">
                <div className="rounded-lg bg-gray-100 p-3 mr-4">
                  <span className="material-icons text-4xl text-gray-700">flight</span>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  <Badge variant="outline" className={`bg-${getStatusColor(selectedDrone.status)}-100 text-${getStatusColor(selectedDrone.status)}-800 border-${getStatusColor(selectedDrone.status)}-300`}>
                    {selectedDrone.status.charAt(0).toUpperCase() + selectedDrone.status.slice(1)}
                  </Badge>
                </div>
                <div>
                  ...
                  <div className="flex items-center">
                    <Progress 
                      ...
                      className="h-2 w-24 mr-2"
                      indicatorClassName={getBatteryColor(selectedDrone.batteryLevel)}
                    />
                    <span className="text-sm font-mono">{selectedDrone.batteryLevel}%</span>
                  </div>
                </div>
                <div>
                  ...
                  <p>
                    {selectedDrone.locationLat && selectedDrone.locationLng ? (
                      <span className="font-mono">
                        {parseFloat(selectedDrone.locationLat).toFixed(6)}, {parseFloat(selectedDrone.locationLng).toFixed(6)}
                      </span>
                    ) : (
                      "No location data"
                    )}
